...
#### EC2初期化スクリプト (user_data.sh)

EC2インスタンスが起動した直後に自動実行される初期化スクリプトです。Ansibleがアプリケーションをインストールする前に、サーバーの基礎的な準備を行います。

**主な役割:**

1. **システムの初期設定**
   - OSのアップデート（`apt-get update/upgrade`）
   - 起動時に一度だけ実行

2. **CloudWatch Logsエージェントのセットアップ**
   - ログ監視エージェントのインストールと設定
   - システムログ（`/var/log/syslog`）とユーザーデータログを自動的にCloudWatchに送信
   - AWSコンソールからリアルタイムでログを確認可能

3. **データボリュームのマウント**
   - 追加のEBSボリューム（50GB）を`/data`にマウント
   - Dockerのデータ（PostgreSQL/Redisのデータ）をこのボリュームに永続化
   - 再起動後も自動マウントされるように`/etc/fstab`に設定

4. **環境変数の設定**
   - S3バケット名とAWSリージョンを`/etc/environment`に保存
   - Ansibleが後でDifyを設定する際に、これらの変数を参照

**実行タイミング:**
```
Terraform → EC2起動 → user_data.sh実行 → Ansible → Difyデプロイ
```

**ログ確認:**
スクリプトの実行ログは `/var/log/user-data.log` に保存され、CloudWatch Logsでも確認できます。
